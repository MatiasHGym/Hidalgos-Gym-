(function () {
  ...
  function renderSchedules() {
    ...
    if (!state.schedules.length) {
      els.scheduleGrid.innerHTML = `<div class="empty-state">Crea el primer horario de boxeo.</div>`;
  ...
  async function init() {
    await window.HidalgoCloud?.requireLogin?.({ title: "Boxeo" });
  ...
  init().catch((error) => {
    console.error(error);
    showToast("No se pudo iniciar Boxeo.");
  });
})();
